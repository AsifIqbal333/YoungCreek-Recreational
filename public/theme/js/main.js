...
(function ($) {
  ...
  -------------------------------------------------------------------------------------*/
    document.addEventListener("DOMContentLoaded", function () {
        ...
        closeButtons.forEach(function (btn) {
            btn.addEventListener("click", function (e) {
                ...
                if (container && !container.classList.contains("is-closed")) {
                    container.classList.add("is-closed");
                    if (menuItem) {
                        menuItem.classList.add("hover-disabled");
                        setTimeout(() => {
                            menuItem.classList.remove("hover-disabled");
                        }, 1000);
                    }
                    setTimeout(() => {
                        container.classList.add("is-hidden");
                        const anyOpen = Array.from(
                            document.querySelectorAll(".sub-menu .container")
        ...
        menuItems.forEach((item) => {
            ...
            if (subMenu) {
                item.addEventListener("mouseenter", () => {
                    subMenu.classList.remove("is-closed", "is-hidden");
                    document.body.classList.add("body--locked"); // 🔒 Заблокувати скрол
                });
            }
        });
        document.addEventListener("click", function (e) {
            ...
            containers.forEach((container) => {
                if (
                    ...
                    !container.classList.contains("is-closed")
                ) {
                    container.classList.add("is-closed");
                    setTimeout(() => {
                        container.classList.add("is-hidden");
                        const anyOpen = Array.from(containers).some(
    ...
    document.addEventListener("DOMContentLoaded", function () {
        const menuLists = document.querySelectorAll(".mega-menu-list");
        menuLists.forEach((menuList) => {
            const megaMenuItems = menuList.querySelectorAll(".mega-menu-item");
            const firstItem = megaMenuItems[0];
            if (firstItem) {
                firstItem.classList.add("hovered");
            }
            megaMenuItems.forEach((item) => {
                item.addEventListener("mouseover", () => {
                    megaMenuItems.forEach((i) => i.classList.remove("hovered"));
                    item.classList.add("hovered");
                });
                item.addEventListener("mouseleave", () => {
                    setTimeout(() => {
                        const isAnyHovered = Array.from(megaMenuItems).some(
                            (i) => i.matches(":hover")
                        );
                        if (!isAnyHovered && firstItem) {
